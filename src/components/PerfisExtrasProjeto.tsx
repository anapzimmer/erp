...
export default function PerfisExtrasProjeto({ perfis, materiais, setMateriais, altura, largura, quantidade }: Props) {
  ...
  return <section className="rounded-2xl border border-border bg-surface p-4" style={{ color: theme.contentTextLightBg }}>
    ...
    {aberto && <div className="mt-4 space-y-3 border-t border-border pt-4">
      ...
      {Number.isFinite(medidaFinal) && medidaFinal > 0 && Number.isInteger(pecasTotais) && pecasTotais > 0 && <p aria-live="polite" className="rounded-lg bg-surface-secondary px-3 py-2 text-sm">
      ...
      <button type="button" className="rounded-lg border border-border-strong px-4 py-2 text-sm hover:bg-surface-secondary" onClick={salvar}>{editando ? "Aplicar alteração" : "Adicionar ao projeto"}</button>
    </div>}
  </section>;
}
