...
type CadastroClientesProps = {
  somenteNovo?: boolean;
  onClose?: () => void;
  onCreated?: (cliente: Cliente) => void;
};

export default function CadastroClientes({ somenteNovo = false, onClose, onCreated }: CadastroClientesProps) {
  ...
  const [mostrarModal, setMostrarModal] = useState(somenteNovo);
  ...
  const salvarCliente = async () => {
    ...
    try {
      ...
      const resultado = editando ? await supabase.from("clientes").update(payload).eq("id", editando.id).eq("empresa_id", empresaId)
        : await supabase.from("clientes").insert([{ ...payload, empresa_id: empresaId }]).select("*").single();

      if (resultado.error) throw resultado.error;

      if (somenteNovo) {
        onCreated?.(resultado.data as Cliente);
        onClose?.();
        return;
      }
      await carregarDados();
  ...
  return (
    <div className={somenteNovo ? "" : "flex min-h-screen"} style={somenteNovo ? undefined : { backgroundColor: theme.screenBackgroundColor }}>
      {!somenteNovo && <>
      <Sidebar
