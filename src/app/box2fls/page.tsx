...
const modeloKitOpcoes = ["Tradicional", "Quadrado", "Elegance"];
...
const desenhoBox2Fls = () => "/desenhos/box-padrao.png";
...
const kitCorrespondeModeloBox = (kit: KitCadastro, modelo: string) => {
  ...
  if (modeloNormalizado.includes("tradicional")) {
    return nome.includes("kit f1") && !nome.includes("quadrado") && !nome.includes("evidence") && !nome.includes("elegance");
  }
  ...
  return (nome.includes("evidence") || nome.includes("elegance")) && nome.includes("frontal");
};
...
export default function Box2FlsPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const alturaMovelMedida = Math.max(0, Number(dados.altura || 0) - (alturaAteTeto ? 25 : 0));
  ...
  const codigosAutomaticos = useMemo(
    () => ["VT806", "VT66"].map(normalizarTexto),
    []
  );
  ...
  useEffect(() => {
    setMateriais((lista) => {
      ...
      return [...itensManuais, ...kitAutomatico, ...perfisAutomaticos];
    });
  }, [codigosAutomaticos, kitAutomatico, perfisAutomaticos]);
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = desenhoBox2Fls();
...
function ProjetoDrawing() {
  const desenhoSrc = desenhoBox2Fls();
