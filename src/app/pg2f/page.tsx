...
export default function PG2FPage() {
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    const regras: Array<{ codigo: string; multiplicador: number; ignorarCor?: boolean }> =
      ...
      : dados.trinco === "Grande"
        ? [
          ...
          { codigo: "1013G", multiplicador: 2 },
        ]
        : [
          { codigo: "1101A", multiplicador: 2 },
          { codigo: "1201A", multiplicador: 2 },
          { codigo: "1103A", multiplicador: 2 },
    ...
    if (dados.trilho === "Sem fechadura") {
      ...
      regras.push(
        ...
        { codigo: "MFLY", multiplicador: 1, ignorarCor: true },
    ...
    const materiaisAgrupados = new Map<string, ProjetoIndividualMaterial>();
    ...
    regras
      ...
      .forEach((item) => {
        const chave = normalizarTexto(`${item.descricao}|${item.unidade}|${item.valorUnitario}`);
        const existente = materiaisAgrupados.get(chave);
        if (existente) {
          materiaisAgrupados.set(chave, { ...existente, qtd: Number(existente.qtd || 0) + Number(item.qtd || 0) });
          return;
        }
        materiaisAgrupados.set(chave, item);
      });
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = obterDesenhoPortaGiro(dados.puxador);
...
function ProjetoDrawing({ comPuxador }: { comPuxador: boolean }) {
  const desenhoSrc = obterDesenhoPortaGiro(comPuxador ? "Com puxador" : "Sem puxador");
