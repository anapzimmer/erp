//app/src/app/(calculos)/jc4fcb/page.tsx
"use client";
...
type JC4FCBOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:jc4fcb:rascunho";
...
const desenhoJanelaBandeira4F = (trinco?: string) =>
  trinco === "Com trinco"
    ? "/desenhos/jc4fcb-comtrinco.png"
    : "/desenhos/jc4fcb-semtrinco.png";
...
export default function JC4FCBPage() {
  ...
  const totalVidros = Number(dados.quantidade || 0) * 8;
  ...
  const larguraBandeiraMedida = Number(dados.largura || 0) / 4;
   ...
   const areaBandeira =(larguraBandeira *alturaBandeira *4 *quantidadeVaos) / 1_000_000;
...
const codigosFerragensAutomaticas = useMemo(
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (dados.trinco === "Com trinco") {
      regras.push({ codigo: "1335", multiplicador: 1 });
    }
  ...
  useEffect(() => {
    setMateriais((lista) => {
      const itensManuais = lista.filter((item) => {
        const descricao = normalizarTexto(item.descricao);
        const ehTuboAutomatico =
          descricao.includes("tubo retangular") ||
          descricao.includes("tubo quadrado");

        const ehItemEliminado = [
          "1519",
          "1038b",
          "1520arou",
          "3530",
        ].some((codigo) => descricao.includes(codigo));

        return (
          !ehTuboAutomatico &&
          !ehItemEliminado &&
          !descricao.includes("kit") &&
          !codigosFerragensAutomaticas.some((codigo) =>
            descricao.includes(codigo)
          )
        );
      });
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as JC4FCBOrcamentoPersistido | null;
    if (itens?.tipo !== "jc4fcb" && itens?.tipo !== "pc4fcb") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao projeto JC4FCB.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: JC4FCBOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento jc4fcb:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
                  ...
                  if (label === "Imprimir") {
                    return (
                      <PDFDownloadLink
                        ...
                        fileName={`jc4fcb_${dados.numero || "novo"}.pdf`}
                        className={itemClass}
                      >
                        {() => (
                          <>
