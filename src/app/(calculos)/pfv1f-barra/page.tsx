...
type PFV1FBarraOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pfv1f-barra:rascunho";
...
const encontrarTuboPadrão = (perfis: PerfilCadastro[]) =>
  perfis.find((perfil) => {
...
export default function PFV1FBarraPage() {
  ...
  const calculoVidro = useMemo(() => {
    const larguraMedida = Number(dados.largura || 0) + 50;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    return [
      ...codigosLargura.map((codigo) => criarPerfilBarra(codigo, largura * 2, 1)),
      codigoAlturaTrilho ? criarPerfilBarra(codigoAlturaTrilho, altura, 1) : null,
      codigoAlturaLateral ? criarPerfilBarra(codigoAlturaLateral, altura, 1) : null,
      tubo,
    ].filter((item): item is ProjetoIndividualMaterial => Boolean(item));
  }, [criarPerfilBarra, dados.altura, dados.corKit, dados.largura, dados.quantidade, dados.vidro, perfilTuboSelecionado]);
  ...
  useEffect(() => {
    ...
    const carregarCadastros = async () => {
      ...
      if (perfisError) {
        ...
        const tuboPadrão = encontrarTuboPadrão(listaPerfis);
        if (tuboPadrão) {
          setPerfilTuboId(tuboPadrão.id);
        }
      }
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => ["3530AROU-CIL", "3530DP", "3230D", "1125A", "1125", "KTJ1", "PUXBC30", "PUXBC60", "PUXBC80", "1519", "1038B", "1520AROU-CIL", "1520P"].map(normalizarTexto),
    []
  );
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    let ferragem = buscarFerragemPorCodigo(codigo, {
        ignorarCor: codigo === "3530AROU-CIL" || codigo === "1520AROU-CIL" || codigo === "1125A" || codigo === "1125" || codigo === "KTJ1",
      });

      if (!ferragem && (codigo.startsWith("PUXBC") || codigo === "1125A" || codigo === "KTJ1")) {
  ...
  const novoProjeto = () => {
    ...
    setMateriais([]);
    setPerfilTuboId(encontrarTuboPadrão(perfis)?.id || null);
  };

  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = dados.puxador === "Com puxador" ? "/desenhos/portaforavao-1flscompleto.png"
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PFV1FBarraOrcamentoPersistido | null;
    if (itens?.tipo !== "pfv1f_barra") {
      setMensagemSistema({
        tipo: "aviso",
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PFV1FBarraOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pfv1f_barra_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
