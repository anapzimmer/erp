...
const encontrarTuboPadrao = (perfis: PerfilCadastro[]) =>
  perfis.find((perfil) => {
...
export default function PFV1FBarraPage() {
  ...
  useEffect(() => {
    ...
    const carregarCadastros = async () => {
      ...
      if (perfisError) {
        ...
        const tuboPadrao = encontrarTuboPadrao(listaPerfis);
        if (tuboPadrao) {
  ...
  useEffect(() => {
    setMateriais((lista) => {
      const itensManuais = lista.filter((item) => !item.codigoPerfil);
      return [...itensManuais, ...perfisAutomaticos];
    });
  }, [perfisAutomaticos]);
  ...
  const novoProjeto = () => {
    ...
    setMateriais([]);
    setPerfilTuboId(encontrarTuboPadrao(perfis)?.id || null);
  };
                  ...
                  if (label === "Imprimir") {
                    return (
                      <PDFDownloadLink
                        ...
                        fileName={`pfv1f_barra_${dados.numero || "novo"}.pdf`}
                        className={itemClass}
                      >
                        {() => (
                          <>
                ...
                <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(330px,400px)_minmax(0,1fr)]">
                  <section className="rounded-2xl border border-white/80 bg-white/95 p-5 shadow-[0_18px_45px_rgba(15,39,66,0.08)]">
                    ...
                    <div className="mt-4 flex min-h-[320px] items-center justify-center rounded-2xl border border-slate-100 bg-gradient-to-br from-white via-slate-50 to-[#eef8f3] p-4 sm:min-h-[420px] xl:min-h-[430px]">
                      <ProjetoDrawing comPuxador={dados.puxador === "Com puxador"} />
                    </div>
                  </section>
