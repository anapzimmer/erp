...
type PC2FCBKitOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc2fcb-kit:rascunho";
...
export default function PC2FCBKitPage() {
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      "VT66", "VT10",
      "3530AROU-CIL", "3530DP", "3230D", "PUXBC30", "PUXBC60", "PUXBC80", "1519", "1038B", "1520AROU-CIL", "1520P",
    ].map(normalizarTexto),
    []
  );
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC2FCBKitOrcamentoPersistido | null;
    if (itens?.tipo !== "pc2fcb_kit") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao PC2FCB - KIT.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC2FCBKitOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento PC2FCB - KIT:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc2fcb_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
