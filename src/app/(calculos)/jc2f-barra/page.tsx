...
type JC2FBarraOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:jc2f-barra:rascunho";
...
export default function JC2FBarraPage() {
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (espessura === 8) regras.push({ codigos: ["1125A"], multiplicador: 2, ignorarCor: true });
    if (espessura === 10) regras.push({ codigos: ["1125A"], multiplicador: 2, ignorarCor: true });
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = dados.trinco === "Com trinco" ? "/desenhos/projeto2f-trinco.png"
    ...
    return {
      ...
      projeto: "Janela de correr 2 folhas - Barra",
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as JC2FBarraOrcamentoPersistido | null;
    if (itens?.tipo !== "jc2f_barra") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento Não pertence ao JC2F - BARRA.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: JC2FBarraOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`jc2f_barra_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ comTrinco }: { comTrinco: boolean }) {
  const desenhoSrc = comTrinco ? "/desenhos/projeto2f-trinco.png" : "/desenhos/projeto2f-simples.png";
