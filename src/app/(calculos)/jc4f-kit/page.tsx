...
export default function JC4FKitPage() {
  ...
  const precoVidroPorNome = useCallback((nomeVidro: string) => {
    const vidro = localizarVidroPorDescricao(vidros, nomeVidro, formatarVidroCadastro);
    if (!vidro) return 0;
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <button
                            ...
                            className="rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-on-primary shadow-sm hover:bg-primary-hover"
                          >
                            Enviar lote PDF+
                          </button>
                            </>
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`jc4f_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
