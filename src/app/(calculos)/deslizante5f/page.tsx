...
type Deslizante5FOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const projetoOpcoes = ["Janela todas correm", "Porta todas correm", "Janela 1 fixo e 4 móveis", "Porta 1 fixo e 4 móveis"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:deslizante5f:rascunho";
...
const desenhoDeslizante5F = (projeto?: string, carrinho?: string, puxador?: string) => {
  ...
  if (projetoComFixo) {
    if (carrinhoInteiro) {
      return comPuxador ? "/desenhos/deslizante-14fls-ci-completo.png" : "/desenhos/deslizante-14fls-ci-simples.png";
    }

    return comPuxador ? "/desenhos/deslizante-14fls-cs-completo.png" : "/desenhos/deslizante-14fls-cs-simples.png";
  }

  if (carrinhoInteiro) {
    return comPuxador ? "/desenhos/deslizante-5fls-ci-completo.png" : "/desenhos/deslizante-5fls-ci-simples.png";
  }

  return comPuxador ? "/desenhos/deslizante-5fls-cs-completo.png" : "/desenhos/deslizante-5fls-cs-simples.png";
};

export default function Deslizante5FPage() {
  ...
  const totalVidros = Number(dados.quantidade || 0) * 5;
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraMovelMedida = (Number(dados.largura || 0) + 40) / 5;
    ...
    const areaFixa = 0;
    const areaMovel = (larguraMovel * alturaMovel * 5 * quantidadeVaos) / 1_000_000;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const larguraVidro = Number(formatarMedidaPeca((largura + 40) / 5));
    const projetoComFixo = normalizarTexto(dados.trilho).includes("fixo");
    const cortesTrilho = projetoComFixo ? 4 : 5;
    ...
    return agruparMateriaisMesmoPerfil([
      ...
      criarPerfilBarra("STY106", altura, 8),
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (!carrinhoInteiro) {
      regras.push({ codigo: "3001", multiplicador: projetoTodasCorrem ? 10 : 8 });
    }
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar orcamento Deslizante 5 folhas:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    const itens = orcamento?.itens as Deslizante5FOrcamentoPersistido | null;
    if (itens?.tipo !== "deslizante5f") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este orçamento não pertence ao Deslizante 5 folhas.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: Deslizante5FOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar orcamento Deslizante 5 folhas:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`deslizante5f_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ projeto, carrinho, comPuxador }: { projeto: string; carrinho: string; comPuxador: boolean }) {
  const desenhoSrc = desenhoDeslizante5F(projeto, carrinho, comPuxador ? "Com puxador" : "Sem puxador");
