...
type JC2FCSKitOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:jc2fcs-kit:rascunho";
...
export default function JC2FCSKitPage() {
  ...
  const [dados, setDados] = useState<JC2FCSDados>({
    projeto: "Janela 2 folhas com sacada inferior - Kit",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const kitSelecionado = useMemo(() => {
    ...
    const kitsOrdenados = kitsFiltrados
      ...
      .sort((a, b) => {
        const prioridadeA = normalizarTexto(a.nome).includes("2f") ? 0 : 1;
        const prioridadeB = normalizarTexto(b.nome).includes("2f") ? 0 : 1;
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      "VT66", "VT10", "1561", "1335", "1038.C", "1038.C-BC",
    ].map(normalizarTexto),
    []
  );
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as JC2FCSKitOrcamentoPersistido | null;
    if (itens?.tipo !== "jc2fcs_kit") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este orçamento não pertence ao JC2FCS - KIT.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: JC2FCSKitOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento JC2FCS - KIT:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`jc2fcs_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
