...
type JC2FCSDados = Omit<ProjetoIndividualDados, "materiais"> & {
  alturaAteTubo: number;
  vidroBandeira: string;
  tuboPerfil: string;
};

type JC2FCSOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
  dados?: Partial<JC2FCSDados>;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:jc2fcs:rascunho";
...
export default function JC2FCSPage() {
  ...
  const [dados, setDados] = useState<JC2FCSDados>({
    projeto: "Janela 2 folhas com sacada inferior",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const calculoVidro = useMemo(() => {
    ...
    const alturaSacada = Math.min(Math.max(0, Number(dados.alturaAteTubo || 0)), alturaBruta);
    const alturaPorta = Math.max(0, alturaBruta - alturaSacada);
    const alturaBandeiraMedida = alturaSacada;
    const larguraBandeiraMedida = Number(dados.largura || 0);
    ...
    const areaBandeira = (larguraBandeira * alturaBandeira * quantidadeVaos) / 1_000_000;
  ...
  const atualizarCampo = <K extends keyof JC2FCSDados>(
    campo: K,
    valor: JC2FCSDados[K]
  ...
  const usaPerfilSacada10mm = (texto: string) =>
    obterEspessuraVidro(texto) === 10 || /0x5\s*\+\s*0x5/.test(normalizarTexto(texto));
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const codigoAlturaUnica = espessura === 10 ? "VT15" : espessura === 8 ? "VT16" : "";
    const codigoUBandeira = usaPerfilSacada10mm(dados.vidroBandeira) ? "VT10" : "VT66";

    return agruparMateriaisMesmoPerfil([
      ...codigosLargura.map((codigo) => criarPerfilBarra(codigo, largura, 1)),
      codigoAlturaDupla ? criarPerfilBarra(codigoAlturaDupla, altura, 2) : null,
      ...
      codigoAlturaUnica ? criarPerfilBarra(codigoAlturaUnica, altura, 1) : null,
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      ...
      "VT51A", "VT52", "VT05", "VT13", "VT10", "VT15",
      "1125A", "KTJ1", "1561", "1335", "1038.C", "1038.C-BC",
    ].map(normalizarTexto),
    []
  );
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    const regras: Array<{ codigos: string[]; multiplicador: number; ignorarCor?: boolean }> = [
      ...
      { codigos: ["KTJ1"], multiplicador: 1 },
    ];

    if (espessura === 8 || espessura === 10) regras.push({ codigos: ["1125A"], multiplicador: 2, ignorarCor: true });
    ...
    return regras
      .map(({ codigos, multiplicador, ignorarCor }) => {
        const ferragem = codigos.map((codigo) => buscarFerragemPorCodigo(codigo, { ignorarCor })).find(Boolean);
  ...
  useEffect(() => {
    ...
    const descricaoVidroBandeira = `VIDRO SACADA ${medidaVidroBandeira} ${vidroBandeiraNome.toUpperCase()}`;
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = dados.trinco !== "Sem trinco" ? "/desenhos/janela-bct-trinco-2fls.png"
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as JC2FCSOrcamentoPersistido | null;
    if (itens?.tipo !== "jc2fcs") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este orçamento não pertence ao JC2FCS.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: JC2FCSOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento JC2FCS:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
                      ...
                      <div className="mt-4 grid gap-3 overflow-visible md:grid-cols-3">
                        ...
                        <label data-campo-legado className="relative flex min-h-[76px] items-center gap-3 rounded-2xl border border-border/80 bg-surface px-4 py-3 transition-colors focus-within:border-success-soft focus-within:bg-surface focus-within:ring-4 focus-within:ring-success/10">
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block text-[10px] font-semibold uppercase tracking-wide text-text-secondary">Vidro janela</span>
                            {listaVidrosAberta ? (
                              <input
                                ref={vidroInputRef}
                                value={dados.vidro}
                        ...
                        <label data-campo-legado className="relative flex min-h-[76px] items-center gap-3 rounded-2xl border border-border/80 bg-surface px-4 py-3 transition-colors focus-within:border-success-soft focus-within:bg-surface focus-within:ring-4 focus-within:ring-success/10">
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block text-[10px] font-semibold uppercase tracking-wide text-text-secondary">Vidro sacada</span>
                            {listaVidrosBandeiraAberta ? (
                              <input
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`jc2fcs_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ comTrinco }: { comTrinco: boolean }) {
  const desenhoSrc = comTrinco ? "/desenhos/janela-bct-trinco-2fls.png"
    : "/desenhos/janela-bst-trinco-2fls.png";
