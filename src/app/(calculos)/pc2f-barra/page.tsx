...
type PC2FBarraOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc2f-barra:rascunho";
...
export default function PC2FBarraPage() {
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const codigosLargura = espessura === 10 ? ["VT51A", "VT52", "VT05", "VT13"]
      : espessura === 8 ? ["VT49A", "VT50A", "VT45", "VT65"]
        : [];
    const codigoAlturaDupla = espessura === 10 ? "VT10" : espessura === 8 ? "VT66" : "";
  ...
  }, [criarPerfilBarra, dados.altura, dados.corKit, dados.largura, dados.vidro]);
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (dados.puxador === "Com puxador") {
      regras.push({ codigo: codigoPuxador, multiplicador: 1, ignorarCor: true });
    }
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = dados.puxador === "Com puxador" && dados.trinco !== "Sem trinco" ? "/desenhos/projeto2fls-trincoepuxador.png"
    ...
    return {
      ...
      projeto: "Porta de correr 2 folhas - Barra",
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC2FBarraOrcamentoPersistido | null;
    if (itens?.tipo !== "pc2f_barra") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao PC2F - BARRA.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC2FBarraOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento PC2F:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc2f_barra_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ comPuxador, comTrinco }: { comPuxador: boolean; comTrinco: boolean }) {
  const desenhoSrc = comPuxador && comTrinco ? "/desenhos/projeto2fls-trincoepuxador.png"
    : comPuxador ? "/desenhos/projeto2f-puxador.png"
      : "/desenhos/projeto2f-simples.png";
