...
const desenhoBoxCanto = (puxador?: string) =>
  puxador === "Com puxador" ? "/desenhos/box-canto4fpd.png" : "/desenhos/box-canto4f.png";
...
export default function BoxCantoPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraFixaAMedida = larguraA / 2;
    const larguraMovelAMedida = larguraFixaAMedida + 50;
    ...
    const larguraMovelA = arredondar5cm(larguraMovelAMedida);
    ...
    const areaMovelA = (larguraMovelA * alturaMovel * quantidadeVaos) / 1_000_000;
    ...
    const areaMovel = areaMovelA + areaMovelB;
    const areaTotalCobrada = areaFixa + areaMovel;

    return {
      larguraCalculo: larguraMovelA,
      alturaCalculo: alturaMovel,
      larguraFixa: larguraFixaA,
      alturaFixa,
      larguraMovel: larguraMovelA,
      alturaMovel,
      larguraFixaMedida: larguraFixaAMedida,
      alturaFixaMedida,
      larguraMovelMedida: larguraMovelAMedida,
      alturaMovelMedida,
      larguraFixaBMedida,
      larguraMovelBMedida,
      areaFixaA: Number(areaFixaA.toFixed(3)),
      areaMovelA: Number(areaMovelA.toFixed(3)),
  ...
  useEffect(() => {
    ...
    const medidaVidroMovelA = `${calculoVidro.larguraMovelMedida}x${calculoVidro.alturaMovelMedida}`;
    ...
    const descricaoVidroMovelA = `VIDRO MOVEL A ${medidaVidroMovelA} ${vidroNome.toUpperCase()}`;
    ...
    setMateriais((lista) => {
      ...
      const vidroMovelA = criarMaterial({
        qtd: calculoVidro.areaMovelA,
        unidade: "m2",
        descricao: descricaoVidroMovelA,
        valorUnitario: precoVidroM2,
      });
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar Orçamento BOXCANTO:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    if (itens?.tipo !== "boxcanto") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao BOXCANTO.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      console.error("Erro ao salvar Orçamento BOXCANTO:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`boxcanto_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
