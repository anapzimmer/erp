...
type FixosOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
  dados?: Partial<FixosDados>;
...
type FixosDados = Omit<ProjetoIndividualDados, "materiais"> & {
  pecasDivisao: number;
};
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:fixos:rascunho";
...
export default function FixosPage() {
  ...
  const [dados, setDados] = useState<FixosDados>({
    projeto: "Fixos",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const totalVidros = Number(dados.quantidade || 0) * Number(dados.pecasDivisao || 1);
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraTotal = Math.max(0, Number(dados.largura || 0) - 25);
    const alturaMedida = Math.max(0, Number(dados.altura || 0) - 25);
    const larguraMedida = pecas > 0 ? larguraTotal / pecas : larguraTotal;
    ...
    const areaTotalCobrada = areaUnit * pecas * Number(dados.quantidade || 0);

    return {
      larguraCalculo,
      alturaCalculo,
      larguraMedida: Math.round(larguraMedida),
      alturaMedida: Math.round(alturaMedida),
      areaTotalCobrada: Number(areaTotalCobrada.toFixed(3)),
    };
  }, [dados.altura, dados.largura, dados.pecasDivisao, dados.quantidade]);
  ...
  const atualizarCampo = <K extends keyof FixosDados>(
    campo: K,
    valor: FixosDados[K]
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const altura = Math.max(0, Number(dados.altura || 0));

    if (dados.corKit === "Escolher" || largura <= 0 || altura <= 0) return [];

    const codigoPerfil = espessura === 10 ? "VT10" : espessura === 8 ? "VT66" : "";
    if (!codigoPerfil) return [];

    const cortesPorProjeto = [largura, largura, altura, altura];
    const perfil = criarPerfilBarra(codigoPerfil, cortesPorProjeto);
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    ...
    const desenhoUrl = desenhoFixosPorPecas(folhas);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as FixosOrcamentoPersistido | null;
    if (itens?.tipo !== "fixos") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao FIXOS.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: FixosOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`fixos_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ pecas }: { pecas: number }) {
