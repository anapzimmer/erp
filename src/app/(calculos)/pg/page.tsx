...
const fechaduraOpcoes = ["Escolher", "Sem fechadura", "1520", "1520TA"];
...
export default function PGPage() {
  ...
  const calculoVidro = useMemo(() => {
    const larguraMedida = Math.max(0, Number(dados.largura || 0) - 15);
  ...
  const codigosItensAutomaticos = useMemo(
    () => ["CT004", "1101A", "1201A", "1103A", "1013A", "1101G", "1201G", "1102G", "1013G", "1133", "1629B", "1520AROU-CIL", "1520P", "1504A", "1520TAROU-CIL", "1520TAP", "MFLY", "1504ATA", "PUXBC30", "PUXBC60", "PUXBC80"].map(normalizarTexto),
    []
  );
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (dados.trilho === "Sem fechadura") {
      regras.push({ codigo: "1629B", multiplicador: 1 });
  ...
  const perfilCT004Automatico = useMemo(() => {
    const quantidadeProjeto = Number(dados.quantidade || 0);
    const largura = Number(dados.largura || 0);
    const altura = Number(dados.altura || 0);

    if (quantidadeProjeto <= 0 || largura <= 0 || altura <= 0) return null;
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = obterDesenhoPortaGiro(dados.trilho, dados.puxador, dados.trinco);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (itens?.tipo !== "pg_1f") {
      setMensagemSistema({
        ...
        mensagem: "Este Orçamento não pertence ao PG - 1 folha.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const dadosAtualizados = {
        ...
        projeto: dados.trinco === "Dobradiça" ? "PG Dobradiça - 1 folha" : "PG - 1 folha",
      };
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pg_1f_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
const obterDesenhoPortaGiro = (tipoFechadura?: string, puxador?: string, tipoFerragem?: string) => {
  const eh1520TA = normalizarTexto(tipoFechadura).includes("1520ta");
  const semFechadura = normalizarTexto(tipoFechadura).includes("sem fechadura");
  const ehDobradica = normalizarTexto(tipoFerragem).includes("dobradica");
  const comPuxador = puxador === "Com puxador";

  if (ehDobradica) {
    if (semFechadura) return "/desenhos/portagirodob-1flssimples.png";
    if (eh1520TA && comPuxador) return "/desenhos/portagirodob-1fls1520tacompleto.png";
    if (eh1520TA) return "/desenhos/portagirodob-1fls1520ta.png";
    if (comPuxador) return "/desenhos/portagirodob-1flscompleto.png";
    return "/desenhos/portagirodob-1fls.png";
  }

  if (eh1520TA && comPuxador) return "/desenhos/portagiro-1fls1520tacompleto.png";
  if (eh1520TA) return "/desenhos/portagiro-1fls1520ta.png";
  if (comPuxador) return "/desenhos/portagiro-1flscompleto.png";
  return "/desenhos/portagiro-1fls.png";
};

function ProjetoDrawing({ tipoFechadura, tipoFerragem, comPuxador }: { tipoFechadura: string; tipoFerragem?: string; comPuxador: boolean }) {
  const desenhoSrc = obterDesenhoPortaGiro(tipoFechadura, comPuxador ? "Com puxador" : "Sem puxador", tipoFerragem);
