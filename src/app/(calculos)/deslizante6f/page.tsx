...
type Deslizante6FOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const projetoOpcoes = ["Janela todas correm", "Porta todas correm", "Janela 1 fixo e 5 móveis", "Porta 1 fixo e 5 móveis"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:deslizante6f:rascunho";
...
const desenhoDeslizante6F = (projeto?: string, carrinho?: string, puxador?: string) => {
  ...
  if (projetoComFixo) {
    if (carrinhoInteiro) {
      return comPuxador ? "/desenhos/deslizante-15fls-ci-completo.png" : "/desenhos/deslizante-15fls-ci-simples.png";
    }

    return comPuxador ? "/desenhos/deslizante-15fls-cs-completo.png" : "/desenhos/deslizante-15fls-cs-simples.png";
  }

  if (carrinhoInteiro) {
    return comPuxador ? "/desenhos/deslizante-6fls-ci-completo.png" : "/desenhos/deslizante-6fls-ci-simples.png";
  }

  return comPuxador ? "/desenhos/deslizante-6fls-cs-completo.png" : "/desenhos/deslizante-6fls-cs-simples.png";
};

export default function Deslizante6FPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const areaFixa = 0;
    const areaMovel = (larguraMovel * alturaMovel * 6 * quantidadeVaos) / 1_000_000;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    return agruparMateriaisMesmoPerfil([
      ...
      criarPerfilBarra("STY106", altura, 10),
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar orcamento Deslizante 6 folhas:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    const itens = orcamento?.itens as Deslizante6FOrcamentoPersistido | null;
    if (itens?.tipo !== "deslizante6f") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este orçamento não pertence ao Deslizante 6 folhas.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: Deslizante6FOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar orcamento Deslizante 6 folhas:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`deslizante6f_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ projeto, carrinho, comPuxador }: { projeto: string; carrinho: string; comPuxador: boolean }) {
  const desenhoSrc = desenhoDeslizante6F(projeto, carrinho, comPuxador ? "Com puxador" : "Sem puxador");
