...
export default function PFV1FKitPage() {
  ...
  useEffect(() => {
    ...
    const carregarCadastros = async () => {
      ...
      if (perfisError) {
        ...
        const tuboPadrão = listaPerfis.find((perfil) => {
  ...
  const novoProjeto = () => {
    ...
    const tuboPadrão = encontrarTuboPadrão();
    setPerfilTuboId(tuboPadrão?.id || null);
  };
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pfv1f_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
