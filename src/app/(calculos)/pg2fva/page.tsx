  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (itens?.tipo !== "pg2fva") {
      setMensagemSistema({
        ...
        mensagem: "Este orçamento não pertence à porta de giro com fixo lateral.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
                  ...
                  if (label === "Imprimir") {
                    return (
                      <PDFDownloadLink
                        ...
                        fileName={`pg2fva_${dados.numero || "novo"}.pdf`}
                        aria-disabled={!medidasValidas}
                        onClick={(event) => { if (!validarMedidas()) event.preventDefault(); }}
                        className={itemClass}
                      >
                        {() => (
                          <>
