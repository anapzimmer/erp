...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pg2fva:rascunho";
...
export default function PG2FVAPage() {
  ...
  const [dados, setDados] = useState<Omit<ProjetoIndividualDados, "materiais">>({
    projeto: "PG2FVA - Porta de giro com fixo lateral",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const larguraFixo = Math.max(0, Number(dados.largura || 0) - Number(dados.alturaAteTubo || 0));
  const medidasValidas = Number(dados.alturaAteTubo || 0) > 15 && larguraFixo > 15 && Number(dados.altura || 0) > 15;
  const validarMedidas = () => {
    if (medidasValidas) return true;
    setMensagemSistema({ tipo: "aviso", titulo: "Confira as medidas", mensagem: "Informe a largura total, a altura e a largura da porta de giro. A porta deve ser menor que o vão, deixando espaço para o fixo; cada peça precisa superar a folga de 15 mm." });
    return false;
  };
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const larguraFixo = Math.max(0, larguraTotal - larguraPorta);
  ...
  }, [buscarPerfilPorCodigo, dados.altura, dados.alturaAteTubo, dados.largura, dados.quantidade, dados.trinco, dados.vidro]);
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    if (!medidasValidas) throw new Error("Confira a largura da porta e do fixo antes de adicionar o projeto.");
              ...
              <div className="flex-1 bg-transparent p-4 sm:p-6 lg:px-8 2xl:px-10">
                <div className="grid grid-cols-1 items-stretch gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(380px,0.85fr)]"><div className="min-w-0"><section className="h-full rounded-xl border border-border bg-surface p-5">
                      ...
                      <div className="mt-4 grid gap-3 overflow-visible md:grid-cols-3">
                        ...
                        <DataInput
                          icon={<MoveHorizontal size={24} strokeWidth={1.6} />}
                          label="Largura da porta de giro"
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pg2fva_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
