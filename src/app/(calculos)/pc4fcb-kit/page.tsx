...
type PC4FCBKitOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc4fcb-kit:rascunho";
...
export default function PC4FCBKitPage() {
  ...
  const [dados, setDados] = useState<PC4FCBDados>({
    projeto: "Porta de correr 4 folhas com bandeira - Kit",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      "VT66", "VT10", "3530AROU-CIL", "3530P", "3534", "PUXBC30", "PUXBC60", "PUXBC80", "1335", "1519", "1038B", "1520AROU-CIL", "1520P",
    ].map(normalizarTexto),
    []
  );
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC4FCBKitOrcamentoPersistido | null;
    if (itens?.tipo !== "pc4fcb_kit") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao pc4fcb - KIT.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC4FCBKitOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento pc4fcb - KIT:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc4fcb_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
