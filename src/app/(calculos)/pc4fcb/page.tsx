...
type PC4FCBDados = Omit<ProjetoIndividualDados, "materiais"> & {
  alturaAteTubo: number;
  vidroBandeira: string;
  tuboPerfil: string;
};

type PC4FCBOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
  dados?: Partial<PC4FCBDados>;
...
const trincoOpcoes = ["Sem trinco", "1335", "1519", "1520", "1335 + 1520", "1519 + 1520"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc4fcb:rascunho";
...
const desenhoPortaBandeira4F = (puxador?: string, trinco?: string) => {
  const temPuxador = puxador === "Com puxador";
  const trincoTexto = String(trinco || "");

  if (temPuxador && trincoTexto.includes("+ 1520")) return "/desenhos/portaband4fls-completa.png";
  if (temPuxador && trincoTexto !== "Sem trinco") return "/desenhos/portaband4fls-puxadoretrinco.png";
  if (temPuxador) return "/desenhos/portaband4fls-puxador.png";
  if (trincoTexto.includes("+ 1520")) return "/desenhos/portaband4fls-trincoechave.png";
  if (trincoTexto !== "Sem trinco") return "/desenhos/portaband4fls-trinco.png";
  return "/desenhos/portaband4fls-simples.png";
};
...
export default function PC4FCBPage() {
  ...
  const [dados, setDados] = useState<PC4FCBDados>({
    projeto: "Porta de correr 4 folhas com bandeira",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const calculoVidro = useMemo(() => {
    ...
    const alturaFixaMedida = Math.max(0, alturaPorta - (ehEmbutido ? 40 : 60));
    const alturaMovelMedida = Math.max(0, alturaPorta - (ehEmbutido ? 0 : 20));
  ...
  const atualizarCampo = <K extends keyof PC4FCBDados>(
    campo: K,
    valor: PC4FCBDados[K]
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const alturaPorta = Number(calculoVidro.alturaPorta || altura);
    const alturaBandeira = Number(calculoVidro.alturaBandeiraMedida || 0);

    if (dados.corKit === "Escolher" || largura <= 0 || altura <= 0) return [];

    const codigosLargura = espessura === 10 ? ["VT51A", "VT52", "VT05", "VT13"]
      : espessura === 8 ? ["VT49A", "VT50A", "VT45", "VT65"]
        : [];
    const codigoAlturaDupla = espessura === 10 ? "VT10" : [8, 6].includes(espessura) ? "VT66" : "";
    const codigoAlturaDuasPecas = espessura === 10 ? "VT15" : espessura === 8 ? "VT16" : "";
    const codigoAlturaUnica = espessura === 10 ? "VT17" : espessura === 8 ? "VT47" : "";
    const codigoUBandeira = espessuraBandeira === 10 ? "VT10" : [8, 6].includes(espessuraBandeira) ? "VT66" : "";

    return agruparMateriaisMesmoPerfil([
      ...codigosLargura.map((codigo) => criarPerfilBarra(codigo, largura, 1)),
      codigoAlturaDupla ? criarPerfilBarra(codigoAlturaDupla, alturaPorta, 2) : null,
      codigoAlturaDuasPecas ? criarPerfilBarra(codigoAlturaDuasPecas, alturaPorta, 2) : null,
      ...
      codigoAlturaUnica ? criarPerfilBarra(codigoAlturaUnica, alturaPorta, 1) : null,
  ...
  }, [calculoVidro.alturaBandeiraMedida, calculoVidro.alturaPorta, criarPerfilBarra, criarPerfilBarraPorCadastro, dados.altura, dados.corKit, dados.largura, dados.vidro, dados.vidroBandeira, perfilTuboSelecionado]);
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    return regras
      ...
      .filter((item): item is ProjetoIndividualMaterial => Boolean(item));
  }, [buscarFerragemPorCodigo, dados.corKit, dados.puxador, dados.quantidade, dados.tamanhoPuxador, dados.trinco, dados.vidro]);
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = desenhoPortaBandeira4F(dados.puxador, dados.trinco);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC4FCBOrcamentoPersistido | null;
    if (itens?.tipo !== "pc4fcb") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao pc4fcb.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC4FCBOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento pc4fcb:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
                      ...
                      <div className="mt-4 grid gap-3 overflow-visible md:grid-cols-3">
                        ...
                        <label data-campo-legado className="relative flex min-h-19 items-center gap-3 rounded-2xl border border-border/80 bg-surface px-4 py-3 transition-colors focus-within:border-success-soft focus-within:bg-surface focus-within:ring-4 focus-within:ring-success/10">
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block text-[10px] font-semibold uppercase tracking-wide text-text-secondary">Vidro porta</span>
                            {listaVidrosAberta ? (
                              <input
                                ref={vidroInputRef}
                                value={dados.vidro}
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc4fcb_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ puxador, trinco }: { puxador?: string; trinco?: string }) {
  const desenhoSrc = desenhoPortaBandeira4F(puxador, trinco);
