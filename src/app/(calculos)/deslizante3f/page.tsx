...
type Deslizante3FOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const projetoOpcoes = ["Janela todas correm", "Porta todas correm", "Janela 1 fixo e 2 móveis", "Porta 1 fixo e 2 móveis"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:deslizante3f:rascunho";
...
const desenhoDeslizante3F = (projeto?: string, carrinho?: string, puxador?: string) => {
  ...
  if (projetoComFixo) {
    if (carrinhoInteiro) {
      return comPuxador ? "/desenhos/deslizante-12fls-ci-completo.png" : "/desenhos/deslizante-12fls-ci-simples.png";
    }

    return comPuxador ? "/desenhos/deslizante-12fls-cs-completo.png" : "/desenhos/deslizante-12fls-cs-simples.png";
  }

  if (carrinhoInteiro) {
    return comPuxador ? "/desenhos/deslizante-3fls-ci-completo.png" : "/desenhos/deslizante-3fls-ci-simples.png";
  }

  return comPuxador ? "/desenhos/deslizante-3fls-cs-completo.png" : "/desenhos/deslizante-3fls-cs-simples.png";
};

export default function Deslizante3FPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraMovelMedida = (Number(dados.largura || 0) + 20) / 3;
    ...
    const areaFixa = 0;
    const areaMovel = (larguraMovel * alturaMovel * 3 * quantidadeVaos) / 1_000_000;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const larguraVidro = Number(formatarMedidaPeca((largura + 20) / 3));
    const projetoComFixo = normalizarTexto(dados.trilho).includes("fixo");
    const cortesTrilho = projetoComFixo ? 2 : 3;
    ...
    return agruparMateriaisMesmoPerfil([
      ...
      criarPerfilBarra("STY106", altura, 4),
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (!carrinhoInteiro) {
      regras.push({ codigo: "3001", multiplicador: projetoTodasCorrem ? 6 : 4});
    }
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar orcamento Deslizante 3 folhas:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    const itens = orcamento?.itens as Deslizante3FOrcamentoPersistido | null;
    if (itens?.tipo !== "deslizante3f") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este orçamento não pertence ao Deslizante 3 folhas.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: Deslizante3FOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar orcamento Deslizante 3 folhas:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
function ProjetoDrawing({ projeto, carrinho, comPuxador }: { projeto: string; carrinho: string; comPuxador: boolean }) {
  const desenhoSrc = desenhoDeslizante3F(projeto, carrinho, comPuxador ? "Com puxador" : "Sem puxador");
