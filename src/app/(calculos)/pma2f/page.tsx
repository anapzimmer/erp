...
const projetoOpcoes = ["Janela", "Porta", "Kit pia"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pma2f:rascunho";
...
const desenhoPma2f = (tipoProjeto?: string, puxador?: string) => {
  if (normalizarTexto(tipoProjeto).includes("kit pia")) return "/desenhos/pma-2fs-kitpia.png";
  return puxador === "Com puxador" ? "/desenhos/pma-2fs-completo.png" : "/desenhos/pma-2fs-simples.png";
};

export default function PMA2FPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraMovelMedida = (Number(dados.largura || 0) / 2) + 50;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const codigosLargura = ["VT68", "VT39"];
    ...
    return [
      ...
      codigoAlturaUnica ? criarPerfilBarra(codigoAlturaUnica, altura, 2) : null,
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      "VT68", "VT39", "VT380", "VT390", "VT10", "VT66",
      "1561", "1122D", "1126AC", "1122M", "1629J",
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    const codigoRoldana = tipoProjeto.includes("kit pia") ? "1122M" : dados.trinco || "1122D";
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = desenhoPma2f(dados.trilho, dados.puxador);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PMA2FOrcamentoPersistido | null;
    if (itens?.tipo !== "pma2f") {
      setMensagemSistema({
        tipo: "aviso",
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PMA2FOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
                      ...
                      <div className="mt-4 grid gap-3 overflow-visible md:grid-cols-3">
                        ...
                        <OptionInput
                          ...
                          onChange={(v) => {
                            ...
                            if (v === "Janela") atualizarCampo("trinco", "1122D");
                            if (v === "Kit pia") atualizarCampo("trinco", "1122M");
                          }}
                        />
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pma2f_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
