...
type PC2FCBDados = Omit<ProjetoIndividualDados, "materiais"> & {
  alturaAteTubo: number;
  vidroBandeira: string;
  tuboPerfil: string;
};

type PC2FCBOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
  dados?: Partial<PC2FCBDados>;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc2fcb:rascunho";
...
export default function PC2FCBPage() {
  ...
  const [dados, setDados] = useState<PC2FCBDados>({
    projeto: "Porta de correr 2 folhas com bandeira",
    numero: "005412",
    data: hojePtBr(),
    cliente: "",
  ...
  const atualizarCampo = <K extends keyof PC2FCBDados>(
    campo: K,
    valor: PC2FCBDados[K]
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      ...
      "1125A", "KTJ1", "3530AROU-CIL", "3530DP", "3230D", "PUXBC30", "PUXBC60", "PUXBC80", "1519", "1038B", "1520AROU-CIL", "1520P",
    ].map(normalizarTexto),
    []
  );
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    const regras: Array<{ codigo: string; multiplicador: number; ignorarCor?: boolean }> = [
      ...
      { codigo: "KTJ1", multiplicador: 1 },
    ];

    if (espessura === 8) regras.push({ codigo: "1125A", multiplicador: 4, ignorarCor: true });
    if (espessura === 10) regras.push({ codigo: "1125A", multiplicador: 4, ignorarCor: true });
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC2FCBOrcamentoPersistido | null;
    if (itens?.tipo !== "pc2fcb") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao PC2FCB.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC2FCBOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento PC2FCB:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc2fcb_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
