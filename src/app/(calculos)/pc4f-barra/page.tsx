...
type PC4FBarraOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pc4f-barra:rascunho";
...
export default function PC4FBarraPage() {
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    return [
      ...
      codigoAlturaDuasPecas ? criarPerfilBarra(codigoAlturaDuasPecas, altura, 2) : null,
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      ...
      "1125A", "KTJ3", "KTK", "3530AROU-CIL", "3530DP", "3530P", "3534", "PUXBC30", "PUXBC60", "PUXBC80", "1335", "1519", "1038B", "1520AROU-CIL", "1520P",
    ].map(normalizarTexto),
    []
  );
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = dados.puxador === "Com puxador" && dados.trinco !== "Sem trinco" ? "/desenhos/porta4fls-completo.png"
    ...
    return {
      ...
      projeto: "Porta de correr 4 folhas - Barra",
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as PC4FBarraOrcamentoPersistido | null;
    if (itens?.tipo !== "pc4f_barra") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao PC4F - BARRA.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PC4FBarraOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`pc4f_barra_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ comPuxador, comTrinco }: { comPuxador: boolean; comTrinco: boolean }) {
  const desenhoSrc = comPuxador && comTrinco ? "/desenhos/porta4fls-completo.png"
    : comPuxador ? "/desenhos/porta4fls-puxador.png"
      : comTrinco ? "/desenhos/porta4fls-comtrincos.png"
        : "/desenhos/porta4fls-simples.png";
