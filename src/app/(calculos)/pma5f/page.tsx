...
const projetoOpcoes = ["Janela todas correm", "Porta todas correm", "Janela 1F + 4M", "Porta 1F + 4M"];
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pma5f:rascunho";
...
const desenhoPMA5F = (tipoProjeto?: string, puxador?: string) => {
  const tipo = normalizarTexto(tipoProjeto);
  const todasCorrem = tipo.includes("todas");
  if (todasCorrem) {
    return puxador === "Com puxador" ? "/desenhos/pma-5fs-completo.png" : "/desenhos/pma-5fs-simples.png";
  }
  return puxador === "Com puxador" ? "/desenhos/pma-14fs-completo.png" : "/desenhos/pma-14fs-simples.png";
};

export default function PMA5FPage() {
  ...
  const calculoVidro = useMemo(() => {
    ...
    const larguraMedida = ((Number(dados.largura || 0) + 40) / 5);
    ...
    const areaTotalCobrada = (larguraCalculo * alturaCalculo * 5 * quantidadeVaos) / 1_000_000;
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const perfisLargura = todasCorrem ? [
  ...
  const codigosFerragensAutomaticas = useMemo(
    () => [
      "VT68", "VT268", "VT39", "VT239", "VT380", "VT390", "VT10", "VT66",
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    const multiplicadorRoldana = todasCorrem ? 10 : 8;
  ...
  useEffect(() => {
    ...
    const descricaoVidroMovel = `VIDRO MOVEL 5 PECAS ${medidaVidroMovel} ${vidroNome.toUpperCase()}`;
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    const desenhoUrl = desenhoPMA5F(dados.trilho, dados.puxador);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar Orçamento PMA5F:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    const itens = orcamento?.itens as PMA5FOrcamentoPersistido | null;
    if (itens?.tipo !== "pma5f") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "Orçamento incompatível",
        mensagem: "Este Orçamento não pertence ao PMA5F.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PMA5FOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar Orçamento PMA5F:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`PMA5F_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
...
function ProjetoDrawing({ tipoProjeto, comPuxador }: { tipoProjeto: string; comPuxador: boolean }) {
  const desenhoSrc = desenhoPMA5F(tipoProjeto, comPuxador ? "Com puxador" : "Sem puxador");
