...
type JC2FKitOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
export default function JC2FKitPage() {
  ...
  const montarItemCentral = (id?: string): CentralImpressaoProjetoItem => {
    ...
    return {
      ...
      projeto: "Janela de correr 2 folhas",
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    const itens = orcamento?.itens as JC2FKitOrcamentoPersistido | null;
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: JC2FKitOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
...
<section className="rounded-xl border border-border bg-surface p-5">
    ...
    <RodapeCalculoProjeto area={numero(calculoVidro.areaTotalCobrada)} pecas={numero(totalVidros, 0)} total={moeda(totalMateriais)} documento={<ProjetoIndividualPDF nomeEmpresa={nomeEmpresa} dados={projetoPdf} logoUrl={logoUsuario} />} arquivo={`jc2f_kit_${dados.numero || "novo"}.pdf`} onEnviar={enviarParaCentralImpressao} onSalvar={salvarOrcamento} salvando={salvandoOrcamento} /></main>
  );
}
