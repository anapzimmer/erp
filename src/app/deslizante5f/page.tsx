...
const agruparMateriaisMesmoPerfil = (materiais: ProjetoIndividualMaterial[]) => {
  ...
  materiais.forEach((material) => {
    ...
    agrupados.set(chave, {
      ...existente,
      qtd: unidade.includes("barra")
        ? calcularBarrasPorCortes(cortes, comprimentoBarra)
...
export default function Deslizante5FPage() {
  ...
  const ferragensAutomaticas = useMemo(() => {
    ...
    if (!carrinhoInteiro) {
      regras.push({ codigo: "3001", multiplicador: projetoTodasCorrem ? 10 : 8, ignorarCor: true });
    }
                  ...
                  if (label === "Imprimir") {
                    return (
                      <PDFDownloadLink
                        ...
                        fileName={`deslizante5f_${dados.numero || "novo"}.pdf`}
                        className={itemClass}
                      >
                        {({ loading }) => (
                          <>
