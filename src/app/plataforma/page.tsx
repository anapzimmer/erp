...
import { Building2, Users, History, Wallet, Layers3, ArrowLeft, ShieldCheck } from "lucide-react";
import SituacaoContaModal, { type AlvoSituacao } from "./SituacaoContaModal";
import { situacoes, dataBrasil, type SituacaoDados, type Situacao } from "@/lib/situacaoConta";
...
export default function PlataformaPage() {
  ...
  return <main className={styles.root}>
    <aside className={styles.sidebar}>
      <div className={styles.brand}><span className={styles.brandIcon}><Layers3 size={23}/></span><div>Glass Code<small>Gestão da plataforma</small></div></div>
      ...
      {painel && aba !== "financeiro" && aba !== "acessos" && <>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          ...
          <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>
            ...
            {aba === 'empresas' && painel.empresas.map(e => <tr key={e.id}><td className="px-5 py-4">{e.nome}</td><td className="px-5 py-4">{e.usuarios}</td><td className="px-5 py-4"><span>{situacoes[e.situacao || (e.bloqueado ? 'suspensa_outro' : 'ativa')]}</span>{e.prazo && <p className="mt-1 text-xs text-slate-500">{e.prazo < dataBrasil() ? "Prazo vencido · conferir" : "Regularizar até"}: {e.prazo.split("-").reverse().join("/")}</p>}{e.bloqueado && <p className="mt-1 max-w-xs text-xs text-slate-500">{e.motivo}</p>}</td><td className="px-5 py-4">{e.protegida ? 'Sua empresa' : <button className={botao} disabled={carregando || salvando} onClick={() => abrir('empresa', e.id, e.nome, e.bloqueado)}>Gerenciar situação</button>}</td></tr>)}
            {aba === 'usuarios' && painel.usuarios.map(u => <tr key={u.id}><td className="px-5 py-4">{u.email}<p className="mt-1 text-xs text-slate-400">{u.confirmado ? 'E-mail confirmado' : 'Confirmação pendente'}</p></td><td className="px-5 py-4">{u.empresa || 'Sem empresa vinculada'}</td><td className="px-5 py-4">{u.empresa_bloqueada ? 'Empresa suspensa' : situacoes[u.situacao || (u.bloqueado ? 'suspensa_outro' : 'ativa')]}{u.bloqueado && <p className="mt-1 max-w-xs text-xs text-slate-500">{u.motivo}</p>}</td><td className="px-5 py-4">{data(u.ultimo_acesso)}</td><td className="px-5 py-4">{u.protegida ? 'Proprietária' : <button className={botao} disabled={carregando || salvando} onClick={() => abrir('usuario', u.id, u.email, u.bloqueado)}>Gerenciar situação</button>}</td></tr>)}
            {aba === 'historico' && painel.historico.map(h => <tr key={h.id}><td className="px-5 py-4 whitespace-nowrap">{data(h.criado_em)}</td><td className="px-5 py-4">{h.alvo_nome}<p className="text-xs text-slate-400">{h.tipo === 'empresa' ? 'Empresa' : 'Usuário'}</p></td><td className="px-5 py-4">{h.situacao ? situacoes[h.situacao] : h.bloqueado ? 'Acesso bloqueado' : 'Acesso liberado'}</td><td className="max-w-md break-words px-5 py-4">{h.motivo}</td></tr>)}
