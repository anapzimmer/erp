...
type Evento = { id: string; email: string; empresa: string | null; acao: string; criado_em: string; origem: string; presenca?: { online: boolean; visto_em: string | null } | null };
...
export default function HistoricoAcessos() {
  const [presencaDisponivel,setPresencaDisponivel]=useState(false);
  ...
  const carregar=useCallback(async () => {
    ...
    try { const r=await consultarPlataforma(`?modo=acessos&busca=${encodeURIComponent(filtro)}&pagina=${pagina}`);if(v===versao.current){setEventos(r.eventos);setPresencaDisponivel(r.presenca_disponivel === true);setErro("");} }
  ...
  return <section className={s.panel}>
    <div className={s.notice}>Entradas e saídas registradas pelo Supabase. Quando não há histórico disponível, mostramos somente o último login conhecido. A coluna Presença agora indica sinal do sistema nos últimos 90 segundos, não o estado na data do registro. Ao fechar ou perder a conexão, pode levar até 2 minutos para atualizar.</div>
    {!presencaDisponivel && !loading && !erro && <p role="status" className={s.notice}>A presença ainda não está disponível. Ative a atualização painel_presenca.sql no Supabase e recarregue o ERP nas sessões abertas.</p>}
    ...
    <div className={s.scroll}><table className={s.table}><thead><tr>{['Data e hora · Brasília','Usuário','Empresa','Presença agora','Registro'].map(t=><th key={t}>{t}</th>)}</tr></thead><tbody>{eventos.map(e=><tr key={e.id}><td>{new Date(e.criado_em).toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo'})}</td><td>{e.email}</td><td>{e.empresa||'Sem empresa vinculada'}</td><td><span className={e.presenca?.online ? 'text-success' : 'text-text-secondary'}>{!e.presenca ? 'Indisponível' : e.presenca.online ? '● Online agora' : 'Sem sinal recente'}</span>{e.presenca?.visto_em && <small>Último sinal: {new Date(e.presenca.visto_em).toLocaleString('pt-BR',{timeZone:'America/Sao_Paulo'})}</small>}</td><td>{nomes[e.acao]||e.acao}<small>{e.origem==='auditoria'?'Histórico de autenticação':'Somente o último login; não é um histórico completo'}</small></td></tr>)}</tbody></table></div>
