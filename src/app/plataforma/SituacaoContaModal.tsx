...
export default function SituacaoContaModal({ alvo,onFechar,onSalvo }: { alvo: AlvoSituacao; onFechar:()=>void; onSalvo:()=>void }) {
  ...
  function trocar(valor:Situacao){
    ...
    setSituacao(nova);setMensagem(mensagens[valor==='pagamento_pendente'?'pagamento_pendente':nova]);
  ...
  function trocarCategoria(valor:keyof typeof categorias){
    ...
    else if(valor==='inadimplencia'&&situacao==='regularizacao'){setPrazo(atual=>atual>=dataBrasil()?atual:prazoRegularizacao());setMensagem(mensagens.pagamento_pendente);}
    setCategoria(valor);
  }
  ...
  return <dialog ref={dialog} className={s.dialog} aria-labelledby="situacao-titulo" onCancel={e=>{if(salvando)e.preventDefault();else onFechar();}}><form noValidate onSubmit={salvar}>
    ...
    <p>{bloqueia?'Ao confirmar, o acesso será interrompido. Os dados serão preservados.':'Ao confirmar, esta restrição não impedirá o acesso. Outras restrições da empresa ou do usuário continuam valendo.'} A alteração começa agora e fica registrada no histórico. O aviso ou bloqueio aparece na sessão aberta do cliente na próxima verificação, em até 15 segundos com a aba visível.</p>
