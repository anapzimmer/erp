...
interface Acabamento {
    ...
    empresa_id: string;
    nome: string;
    // --- ALTERADO: Adicionado 'm2' ---
    tipo_calculo: "unitário" | "metro_linear" | "porcentagem" | "m2";
    // ---------------------------------
    preco: number;
    tipo_visual: string; // Ex: 'lapidado-quadrado'
...
export default function AcabamentosPage() {
    ...
    const [novoAcabamento, setNovoAcabamento] = useState({
        ...
        preco: 0,
        tipo_visual: 'lapidado' as Acabamento['tipo_visual'], // Valor inicial válido
        sobra_largura: 0,
        sobra_altura: 0,
        empresa_id: '', // Importante incluir
        id: 0 // Importante incluir
    });
    ...
    const salvarAcabamento = async () => {
        try {
            setCarregando(true);
            if (!empresaIdUsuario) throw new Error("Empresa não identificada");

            // 1. GARANTIR FORMATO DO TIPO_VISUAL
            let tipoVisualFinal = novoAcabamento.tipo_visual || 'lapidado-quadrado';

            // 2. PREPARAR DADOS (SEM SVG)
    ...
    return (
        <div className="flex min-h-screen" style={{ backgroundColor: theme.bgLight }}>
            ...
            <div className="flex-1 flex flex-col w-full min-w-0">
                <header className="border-b border-gray-100 py-3 px-4 md:py-4 md:px-8 flex items-center justify-between sticky top-0 z-30 shadow-sm" style={{ backgroundColor: theme.secondary }}>
                        ...
                        <div className="relative" ref={userMenuRef}>
                            ...
                            {showUserMenu && (
                                <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 p-2 z-50">
                                    <div className="px-3 py-2 border-b border-gray-100"><p className="text-xs text-gray-400 font-medium">Logado como</p><p className="text-sm font-semibold text-gray-900 truncate">{usuarioEmail}</p></div>
                                    <button onClick={handleLogout} className="flex w-full items-center gap-2.5 px-3 py-2.5 text-sm text-red-600 hover:bg-red-50 rounded-xl transition-colors"><X size={18} className="text-red-500" /> Sair</button>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <main className="p-4 md:p-8 flex-1">
                    ...
                    <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
                        ...
                        }} className="flex items-center gap-2 px-6 py-2.5 rounded-2xl font-bold text-xs tracking-wider shadow-sm transition-all hover:scale-[1.02] active:scale-95" style={{ backgroundColor: theme.tertiary, color: theme.primary }}>
                                        ...
                                        <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="p-4">
                                                <div className="flex justify-center gap-2">
                                                    <button onClick={() => { setEditando(s); setNovoAcabamento(s); setMostrarModal(true); }} className="p-2.5 rounded-xl hover:bg-gray-100" style={{ color: theme.primary }}><Edit2 size={18} /></button>
                                                    <button onClick={() => { setAcabamentoParaExcluir(s); setMostrarModalExclusao(true); }}
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    >
                                        ...
                                        <option value="m2">Metro Quadrado (R$/m²)</option> {/* <--- Adicionado */}
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                // Pega o formato atual (ex: 'quadrado') ou usa 'quadrado' como padrão
                                                const formato = novoAcabamento.tipo_visual?.split('-')[1] || 'quadrado';
                                                setNovoAcabamento({ ...novoAcabamento, tipo_visual: `lapidado-${formato}` as any })
                                            }}
                                            className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all 
                                ${novoAcabamento.tipo_visual?.startsWith('lapidado') ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}
                                        >
                                        ...
                                        <button
                                            ...
                                            onClick={() => {
                                                const formato = novoAcabamento.tipo_visual?.split('-')[1] || 'quadrado';
                                                setNovoAcabamento({ ...novoAcabamento, tipo_visual: `bisote-${formato}` as any })
                                            }}
                                            className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all 
                                ${novoAcabamento.tipo_visual?.startsWith('bisote') ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}
                                        >
                                ...
                                {/* 2. FORMATO DO ESPELHO (CORRIGIDO) */}
                                <div>
                                    <label className="text-xs font-bold text-gray-400 uppercase ml-1 mb-3 block">
                                        Formato do Espelho
                                    </label>
                                    <div className="grid grid-cols-4 gap-3">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => {
                                                // CORREÇÃO: Define corretamente como 'jogo'
                                                const bordaBase = novoAcabamento.tipo_visual?.startsWith('bisote') ? 'bisote' : 'lapidado';
                                                setNovoAcabamento({ ...novoAcabamento, tipo_visual: `${bordaBase}-jogo` as any })
                                            }}
                                            className={`col-span-2 flex flex-col items-center gap-3 p-4 rounded-2xl border-2 transition-all 
    ${novoAcabamento.tipo_visual?.endsWith('jogo') // CORREÇÃO: Verifica se termina com 'jogo'
                                                    ? 'border-blue-500 bg-blue-50'
                                                    : 'border-gray-100 hover:border-gray-200 bg-gray-50'}`}
                                        >
                                            <div className="grid grid-cols-3 gap-1 p-2 bg-white rounded-lg">
                                                {[...Array(9)].map((_, i) => (
                                                    <div key={i} className={`w-5 h-5 bg-gray-200 rounded-sm ${novoAcabamento.tipo_visual?.startsWith('bisote') ? 'border-[3px] border-double border-gray-300' : 'border border-gray-300'}`}></div>
                                                ))}
                                            </div>
                                        ...
                                        {/* DEMAIS FORMATOS */}
                                        {opcoesVisual
                                            // CORREÇÃO AQUI: Filtra os nomes que NÃO existem no banco de dados
                                            ...
                                            .map((opt) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        type="button"
                                                        onClick={() => {
                                                            // Garante que o formato novo respeite a nomenclatura do banco (borda-formato)
                                                            setNovoAcabamento({ ...novoAcabamento, tipo_visual: `${bordaBase}-${opt.value}` as any })
                                                        }}
                        ...
                        <div className="p-5 border-t border-gray-100 bg-white flex gap-3">
                            ...
                            <button
                                ...
                                className="flex-1 py-2.5 text-sm font-semibold rounded-xl transition-all flex justify-center items-center gap-2 hover:opacity-90"
                                style={{ backgroundColor: theme.tertiary, color: theme.primary }}
                            >
                                {carregando ? <Loader2 className="animate-spin" size={16} /> : "Salvar"}
                            </button>
                        </div>
                    </div>
                </div>
