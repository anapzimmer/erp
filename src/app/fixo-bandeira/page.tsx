...
export default function FixoBandeiraPage() {
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Nao foi possivel restaurar as configuracoes do projeto:", erro);
    }
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      console.warn("Nao foi possivel salvar as configuracoes do projeto:", erro);
    }
  }, [preferencias]);
  ...
  const novoProjeto = () => {
    ...
    setDados((atual) => ({
      ...
      largura: 0,
      altura: 0,
      quantidade: preferencias.quantidadePadrao,
      trilho: "",
      vidro: "Escolher",
      vidroBandeira: "Escolher",
      corKit: preferencias.corPadrao,
      alturaAteTubo: 0,
      tuboPerfil: "Escolher",
      tuboUso: "Somente largura",
      puxador: "",
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            <OptionInput
              ...
              label="Cor padrão"
              value={preferencias.corPadrao}
              options={corKitOpcoes}
              onChange={(v) => setPreferencias((atual) => ({ ...atual, corPadrao: v }))}
            />
            <DataInput
              icon={<Copy size={22} strokeWidth={1.6} />}
              label="Quantidade padrão"
              value={preferencias.quantidadePadrao}
              onChange={(v) => setPreferencias((atual) => ({ ...atual, quantidadePadrao: Math.max(1, Number(v || 1)) }))}
            />
          </div>
