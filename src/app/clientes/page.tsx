...
import {
  Users, Phone, MapPin, Map, X, Trash2, Edit2, PlusCircle, Search, Building2, ChevronDown, LogOut, Settings, Menu, UsersRound,
} from "lucide-react"
    ...
    <div className="flex min-h-screen text-gray-900" style={{ backgroundColor: theme.screenBackgroundColor }}>
        ...
        <main className="p-4 md:p-8 flex-1">
          ...
          <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
            <div className="flex flex-wrap gap-3">
              ...
              <select value={filtroCidade} onChange={e => setFiltroCidade(e.target.value)} className="p-2.5 rounded-xl border border-gray-200 text-sm bg-white focus:ring-1" style={{ borderColor: theme.menuIconColor }}>
                  ...
                  <tr key={cliente.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => abrirModalParaEdicao(cliente)} className="p-2.5 rounded-xl hover:bg-gray-100" style={{ color: theme.menuBackgroundColor }} title="Editar"> <Edit2 size={18} /> </button>
      ...
      {showScrollTop && (
        <button
          ...
          style={{ backgroundColor: theme.menuIconColor, color: theme.menuBackgroundColor }}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 animate-fade-in px-4">
          <div className="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-lg border border-gray-100">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-extrabold" style={{ color: theme.menuBackgroundColor }}>{editando ? "Editar Cliente" : "Cadastrar Cliente"}</h2>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="col-span-2">
                ...
                <input type="text" placeholder="Ex: João Silva" value={novoCliente.nome} onChange={e => setNovoCliente({ ...novoCliente, nome: e.target.value })} className="w-full p-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:outline-none" style={{ "--tw-ring-color": theme.menuIconColor } as React.CSSProperties} />
              </div>

              <div>
                <label className="text-sm font-semibold text-gray-600 mb-1 block">Telefone</label>
                <input type="text" placeholder="(00) 00000-0000" value={novoCliente.telefone ?? ""} onChange={e => handleTelefoneChange(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:outline-none" style={{ "--tw-ring-color": theme.menuIconColor } as React.CSSProperties} />
              </div>

              <div>
                <label className="text-sm font-semibold text-gray-600 mb-1 block">Cidade</label>
                <input type="text" placeholder="Ex: Cidade" value={novoCliente.cidade ?? ""} onChange={e => setNovoCliente({ ...novoCliente, cidade: e.target.value })} className="w-full p-3 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:outline-none" style={{ "--tw-ring-color": theme.menuIconColor } as React.CSSProperties} />
              </div>
            ...
            <div className="flex gap-3 justify-end pt-4 border-t border-gray-100">
              ...
              <button onClick={salvarCliente} disabled={carregando} className="px-6 py-3 rounded-xl text-sm font-semibold text-white transition hover:opacity-90" style={{ backgroundColor: theme.menuBackgroundColor }}>
