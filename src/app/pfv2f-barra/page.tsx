...
type PFV2FBarraOrcamentoPersistido = {
  tipo?: string;
  modo?: string;
...
const PROJETO_INDIVIDUAL_DRAFT_KEY = "glasscode:pfv2f-barra:rascunho";
...
export default function PFV2FBarraPage() {
  ...
  const perfisAutomaticos = useMemo(() => {
    ...
    const codigoAlturaEncontro = espessura === 10 ? "VT17" : espessura === 8 ? "VT47" : "";

    return [
      ...codigosLargura.map((codigo) => criarPerfilBarra(codigo, largura * 2, 1)),
      codigoAlturaLateral ? criarPerfilBarra(codigoAlturaLateral, altura, 2) : null,
      codigoAlturaEncontro ? criarPerfilBarra(codigoAlturaEncontro, altura, 1) : null,
  ...
  const carregarOrcamentoParaEdicao = useCallback(async () => {
    ...
    if (error) {
      console.error("Erro ao carregar orÃ§amento PFV2F:", error);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao carregar",
    ...
    const itens = orcamento?.itens as PFV2FBarraOrcamentoPersistido | null;
    if (itens?.tipo !== "pfv2f_barra") {
      setMensagemSistema({
        tipo: "aviso",
        titulo: "OrÃ§amento incompatÃ­vel",
        mensagem: "Este orÃ§amento nÃ£o pertence ao PFV2F - BARRA.",
        aoFechar: () => router.push(returnTo),
      });
      return;
    }
  ...
  const salvarOrcamento = async () => {
    ...
    try {
      ...
      const itensPersistidos: PFV2FBarraOrcamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
      ...
      console.error("Erro ao salvar orÃ§amento PFV2F:", erro);
      setMensagemSistema({
        tipo: "erro",
        titulo: "Erro ao salvar",
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        ...
                        <div className="flex items-center gap-2 opacity-0 transition-opacity hover:opacity-100 focus-within:opacity-100">
                          ...
                          <PDFDownloadLink
                            ...
                            fileName={`pfv2f_barra_${dados.numero || "novo"}.pdf`}
