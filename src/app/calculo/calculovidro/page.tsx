...
const extrairMedida = (medida: string) => {
  const [largura, altura] = String(medida || "")
    .split("x")
    .map((valor) => Number(String(valor).replace(/\D/g, "")) || 0);
...
useEffect(() => {
  ...
  const enviarParaCentralImpressao = () => {
    ...
    const clienteNome = listaClientes.find((c) => String(c.id) === String(clienteId))?.nome || "";
    const totalPecas = itens.reduce((acc, item) => acc + Number(item.qtd || 0), 0);
    const valorTotal = itens.reduce((acc, item) => acc + Number(item.total || 0), 0);
    const areaTotal = itens.reduce((acc, item) => {
    ...
    const materiais = itens.map((item) => {
      ...
      const ehVidro = largura > 0 && altura > 0;
      const valorUnitario = ehVidro && area > 0
        ? Number(item.total || 0) / area
        : Number(item.valorUnitario || item.valorServicoUn || 0);
    ...
    try {
      ...
      setModalAvisoMensagem("Não foi possível enviar os vidros para a central de impressão.");
      setMostrarModalAviso(true);
    }
  };
