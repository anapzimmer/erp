...
type SacadaFrontalDraft = {
  ...
  movimentacaoFolhas: MovimentacaoMaoAmiga;
  temPuxador: boolean;
  buscaVidro: string;
  vidroId: string;
  corPerfil: string;
...
type MovimentacaoMaoAmiga = "todas_correm" | "uma_fixa";

const CORES_PERFIL = ["Branco", "Preto", "Fosco"];
const OPCOES_FOLHAS_MAO_AMIGA = ["2", "3", "4", "5", "6"];
const LABEL_MOVIMENTACAO_MAO_AMIGA: Record<MovimentacaoMaoAmiga, string> = {
  todas_correm: "Todas correm",
  uma_fixa: "1 fixa e correm as outras",
};
const SACADA_FRONTAL_DRAFT_KEY = "mao-amiga-draft";
...
export default function CalculoMaoAmigaPage() {
  ...
  const [quantidadeFolhas, setQuantidadeFolhas] = useState("3");
  const [movimentacaoFolhas, setMovimentacaoFolhas] = useState<MovimentacaoMaoAmiga>("todas_correm");
  const [temPuxador, setTemPuxador] = useState(false);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof draft.quantidadeFolhas === "string") setQuantidadeFolhas(draft.quantidadeFolhas);
      if (draft.movimentacaoFolhas === "todas_correm" || draft.movimentacaoFolhas === "uma_fixa") {
        setMovimentacaoFolhas(draft.movimentacaoFolhas);
      }
      if (typeof draft.temPuxador === "boolean") setTemPuxador(draft.temPuxador);
      ...
      console.warn("Nao foi possivel restaurar rascunho da Mão Amiga:", error);
    } finally {
      setDraftHidratado(true);
    }
  }, [chaveDraft, editId]);
  ...
  useEffect(() => {
    ...
    const carregarInsumos = async () => {
      ...
      if (resVidros.error || resPerfis.error || resFerragens.error) {
        if (resVidros.error) {
          console.error("Erro ao carregar vidros da Mão Amiga:", resVidros.error);
        }
        if (resPerfis.error) {
          console.error("Erro ao carregar perfis da Mão Amiga:", resPerfis.error);
        }
        if (resFerragens.error) {
          console.error("Erro ao carregar ferragens da Mão Amiga:", resFerragens.error);
        }
        setVidros([]);
        setPerfisTabela([]);
        setFerragensTabela([]);
  ...
  const carregarOrcamentoParaEdicao = useCallback(async (id: string) => {
    try {
      ...
      if (itensData.quantidadeFolhas != null) setQuantidadeFolhas(String(itensData.quantidadeFolhas));
      else if (itensData.divisoesPorVao != null) setQuantidadeFolhas(String(itensData.divisoesPorVao));
      const movimentacaoEditada = String(itensData.movimentacaoFolhas || "");
      if (movimentacaoEditada === "todas_correm" || movimentacaoEditada === "uma_fixa") {
        setMovimentacaoFolhas(movimentacaoEditada);
      }
      if (typeof itensData.temPuxador === "boolean") setTemPuxador(itensData.temPuxador);
  ...
  const quantidadeFolhasNumero = Math.max(Number(quantidadeFolhas) || 0, 1);
  const quantidadeDivisoesNumero = quantidadeFolhasNumero;
  const movimentacaoLabel = LABEL_MOVIMENTACAO_MAO_AMIGA[movimentacaoFolhas];
  const puxadorLabel = temPuxador ? "Com puxador" : "Sem puxador";
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (!editId) {
        ...
        const prefixoData = `MAO${dataAtual.getFullYear().toString().slice(-2)}${(dataAtual.getMonth() + 1).toString().padStart(2, "0")}`;
      ...
      const dadosParaSalvar = {
        ...
        itens: {
          ...
          alturaVaoMm: alturaNumero,
          quantidadeVaos: quantidadeNumero,
          quantidadeFolhas: quantidadeFolhasNumero,
          movimentacaoFolhas,
          movimentacaoDescricao: movimentacaoLabel,
          temPuxador,
          puxadorDescricao: puxadorLabel,
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  ...
                  setAlturaVaoMm("");
                  setQuantidadeVaos("");
                  setQuantidadeFolhas("3");
                  setMovimentacaoFolhas("todas_correm");
                  setTemPuxador(false);
                  setBuscaVidro("");
                  setVidroId("");
                  setCorPerfil("");
              ...
              <PDFDownloadLink
                document={
                  <SacadaFrontalPDF
                    tituloDocumento="Orçamento Mão Amiga"
                    nomeEmpresa={nomeEmpresa}
                ...
                fileName={`Mão Amiga ${nomeClienteSelecionado || "Geral"} - ${Date.now().toString().slice(-6)}.pdf`}
              >
                {({ loading: pdfLoading }) => (
                  <button
                    className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-bold uppercase tracking-wider transition-all active:scale-95 border shadow-sm"
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 w-full md:w-auto md:min-w-105">
                ...
                <label className="rounded-2xl border p-4" style={{ borderColor: `${theme.contentTextLightBg}12`, backgroundColor: theme.screenBackgroundColor }}>
                  ...
                  <select
                    value={quantidadeFolhas}
                    onChange={(e) => setQuantidadeFolhas(e.target.value)}
                    className="mt-3 w-full bg-transparent text-2xl font-black outline-none"
                    style={{ color: theme.contentTextLightBg }}
                  >
                    {OPCOES_FOLHAS_MAO_AMIGA.map((opcao) => (
                      <option key={opcao} value={opcao} className="text-slate-900">
                        {opcao} folhas
                      </option>
                    ))}
                  </select>
                ...
                <div className="rounded-2xl border p-4 sm:col-span-2 xl:col-span-2" style={{ borderColor: `${theme.contentTextLightBg}12`, backgroundColor: theme.screenBackgroundColor }}>
                  <span className="text-[11px] uppercase tracking-[0.16em] font-bold" style={{ color: `${theme.contentTextLightBg}80` }}>
                    Movimentação
                  </span>
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {(Object.keys(LABEL_MOVIMENTACAO_MAO_AMIGA) as MovimentacaoMaoAmiga[]).map((opcao) => {
                      const ativo = movimentacaoFolhas === opcao;
                      return (
                        <button
                          key={opcao}
                          type="button"
                          onClick={() => setMovimentacaoFolhas(opcao)}
                          className="px-3 py-2 rounded-xl text-xs font-black border-2 transition-all"
                          style={ativo
                            ? { backgroundColor: theme.menuIconColor, color: "#fff", borderColor: theme.menuIconColor }
                            : { backgroundColor: "transparent", color: theme.contentTextLightBg, borderColor: `${theme.contentTextLightBg}24` }}
                        >
                          {LABEL_MOVIMENTACAO_MAO_AMIGA[opcao]}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div className="rounded-2xl border p-4" style={{ borderColor: `${theme.contentTextLightBg}12`, backgroundColor: theme.screenBackgroundColor }}>
                  <span className="text-[11px] uppercase tracking-[0.16em] font-bold" style={{ color: `${theme.contentTextLightBg}80` }}>
                    Puxador
                  </span>
                  <button
                    type="button"
                    onClick={() => setTemPuxador((atual) => !atual)}
                    className="mt-3 w-full px-3 py-2 rounded-xl text-xs font-black border-2 transition-all"
                    style={temPuxador
                      ? { backgroundColor: theme.menuIconColor, color: "#fff", borderColor: theme.menuIconColor }
                      : { backgroundColor: "transparent", color: theme.contentTextLightBg, borderColor: `${theme.contentTextLightBg}24` }}
                  >
              ...
              <article className="rounded-4xl border p-6 shadow-sm" style={{ backgroundColor: theme.contentTextDarkBg, borderColor: `${theme.contentTextLightBg}10` }}>
                ...
                <div className="mt-4">
                  {(() => {
                    ...
                    const mostrarAcessoriosMoveis = movimentacaoFolhas === "todas_correm";
                    const corPerfilNorm = normalizarTextoComparacao(corPerfil);
                    const acessorioStroke = corPerfilNorm === "preto" ? "#f8fafc" : "#1f2937";
                    const acessorioFill = corPerfilNorm === "preto" ? "#111827" : "#f8fafc";
                    ...
                    return (
                      <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {Array.from({ length: divisoes }).map((_, i) => {
                          ...
                          const gX = pX + postW;
                          const centroX = gX + glassW / 2;
                          const roldanaY = y0 + railH + 4;
                          const puxadorX = gX + glassW - Math.max(5, glassW * 0.12);
                          const puxadorTop = y0 + railH + glassH * 0.34;
                          const puxadorBottom = y0 + railH + glassH * 0.66;
                          return (
                            <g key={i}>
                              {/* Pontalete */}
                              ...
                              {mostrarAcessoriosMoveis && (
                                <g>
                                  <circle cx={centroX - Math.min(7, glassW * 0.12)} cy={roldanaY} r={2.4} fill={acessorioFill} stroke={acessorioStroke} strokeWidth={0.8} />
                                  <circle cx={centroX + Math.min(7, glassW * 0.12)} cy={roldanaY} r={2.4} fill={acessorioFill} stroke={acessorioStroke} strokeWidth={0.8} />
                                </g>
                              )}

                              {temPuxador && (
                                <line x1={puxadorX} y1={puxadorTop} x2={puxadorX} y2={puxadorBottom} stroke={acessorioStroke} strokeWidth={2.2} strokeLinecap="round" opacity={0.85} />
                              )}
                        ...
                        {mostrarAcessoriosMoveis && (
                          <g>
                            <rect x={x0 + 3} y={y0 + railH + glassH * 0.42} width={6} height={18} rx={1.4} fill={acessorioFill} stroke={acessorioStroke} strokeWidth={0.9} />
                            <circle cx={x0 + 6} cy={y0 + railH + glassH * 0.58} r={1.1} fill={acessorioStroke} />
                            <rect x={x0 + drawW - 9} y={y0 + railH + glassH * 0.42} width={6} height={18} rx={1.4} fill={acessorioFill} stroke={acessorioStroke} strokeWidth={0.9} />
                            <circle cx={x0 + drawW - 6} cy={y0 + railH + glassH * 0.58} r={1.1} fill={acessorioStroke} />
                            <text x={x0 + drawW / 2} y={y0 + railH + 12} textAnchor="middle" fontSize={7} fill={theme.contentTextLightBg} opacity={0.55} fontWeight={700} fontFamily="system-ui, sans-serif">
                              Roldanas superiores e fechaduras nas pontas
                            </text>
                          </g>
                        )}
                ...
                <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
                  ...
                  <span className="text-xs" style={{ color: `${theme.contentTextLightBg}50` }}>
                    {quantidadeFolhasNumero} {quantidadeFolhasNumero === 1 ? "folha" : "folhas"} por vão · {movimentacaoLabel}
                  </span>
                </div>
              </article>
              ...
              <article className="rounded-4xl border p-6 shadow-sm" style={{ backgroundColor: theme.contentTextDarkBg, borderColor: `${theme.contentTextLightBg}10` }}>
                ...
                <div className="mt-5 space-y-4">
                  {[
                    ...
                    ["Quantidade de folhas", `${quantidadeFolhasNumero} por vão`],
                    ["Movimentação", movimentacaoLabel],
                    ["Puxador", puxadorLabel],
