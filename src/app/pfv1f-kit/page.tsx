...
type PFV1FKitOrçamentoPersistido = {
  tipo?: string;
  modo?: string;
...
export default function PFV1FKitPage() {
  ...
  const [salvandoOrçamento, setSalvandoOrçamento] = useState(false);
  ...
  useEffect(() => {
    if (!rascunhoRestaurado || editId) return;
  ...
  }, [dados, editId, materiais, perfilTuboId, rascunhoRestaurado]);
  ...
  const gerarNumeroOrçamento = async () => {
  ...
  const carregarOrçamentoParaEdicao = useCallback(async () => {
    if (!editId) return;
    ...
    const itens = orcamento?.itens as PFV1FKitOrçamentoPersistido | null;
  ...
  const salvarOrçamento = async () => {
    ...
    try {
      setSalvandoOrçamento(true);

      const numeroFinal = editId ? dados.numero : await gerarNumeroOrçamento();
      ...
      const itensPersistidos: PFV1FKitOrçamentoPersistido & {
        resumo: {
          areaTotal: number;
          totalVidros: number;
          valorVidros: number;
              ...
              <nav className="flex flex-1 flex-row gap-3 overflow-x-auto px-3 py-3 lg:flex-col lg:gap-4 lg:overflow-visible lg:px-4 lg:py-5">
                {[
                  ...
                  <button
                    ...
                    disabled={label === "Salvar" && salvandoOrçamento}
                  ...
                  >
                    <Icon size={22} />
                    <span className="lg:hidden xl:inline">{label === "Salvar" && salvandoOrçamento ? "Salvando..." : label}</span>
                  </button>
                ))}
              </nav>
            </aside>
